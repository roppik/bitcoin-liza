...
#if defined(NDEBUG)
# error "BitcoinLiza cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x1fce1737b6b8f0ffc6f5453bbcf07c2f46a98dc0092c16f1b0a5d6a82984c630");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // BitcoinLiza: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "BitcoinLiza Signed Message:\n";
...
{
    // BitcoinLiza: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 10000); // BitcoinLiza: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // BitcoinLiza: 1 days
static const int64 nTargetSpacing = 2.5 * 60; // BitcoinLiza: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // BitcoinLiza: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // BitcoinLiza: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xd7d9c078630feb5b02ee4db3cb7bba98f48130f894ef885a82f68a302a280d8b");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Bitcoin-Liza Salamon Kix";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 150 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04d4b4278bb49be64c7753217c21f586c32cd6ab65873473fe73561da704bad27e3a5db78a93266fbfa2cb3b3681f7a2c0cd57416314bff26dcd6a0375f4b855bf") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1548264457;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2086463251;

        if (fTestNet)
        {
            block.nTime    = 1548264481;
            block.nNonce   = 386358890;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0x1fce1737b6b8f0ffc6f5453bbcf07c2f46a98dc0092c16f1b0a5d6a82984c630"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfa, 0xc8, 0xb7, 0xda }; // BitcoinLiza: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("BitcoinLizaMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("BitcoinLizaMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static BitcoinLizaMiner(CWallet *pwallet)
{
    printf("BitcoinLizaMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("bitcoin-liza-miner");
    ...
    try { loop {
        ...
        printf("Running BitcoinLizaMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("BitcoinLizaMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&BitcoinLizaMiner, pwallet));
}
